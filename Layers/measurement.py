...
def Projector_Photon_detection_Post_Processing(n, m, modes_detected, modes_detected_post_selection, values, device):
    """
    Args:
        - n (int): number of photons in the system
        - m (int): number of modes in the system
...
class Measure_Projector_Fock_basis(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        return torch.stack([torch.matmul(self.P.T,torch.diag(density_matrix)) for density_matrix in input]).to(self.device) # Get the diagonal of the density matrix
...
class Measure_Photon_detection_Post_Selection(nn.Module):
    """ This class describes the action of a measurement that detect if photons are in a mode corresponding to a label."""
    def __init__(self, n, m, modes_detected, modes_detected_post_selection, values, device):
        """ Args:
            - n: number of photons in the system
            - m: number of modes in the system
            - modes_detected (list): list of modes (int) corresponding to the label
        ...
        if len(values) == 0:
            ...
            self.P = Projector_Photon_detection_Post_Processing(n, m, modes_detected, modes_detected_post_selection, values, device)
...
class Measure_Possible_Outcome_SI(nn.Module):
    ...
    def forward(self, input):
        ...
        Output:
            - density matrix of the measured state
        """
        return torch.stack([torch.matmul(self.Projector.T,torch.diag(density_matrix)) for density_matrix in input]).to(self.device)
