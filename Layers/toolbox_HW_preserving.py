...
def Give_Image_Basis(d1, d2):
    """ This function gives the basis of the sates used to encoded
    an image of size (d1, d2) in a dictionnary.
    Args:
        - d1: first size of the image
        - d2: second size of the image
    Output:
        - a dictionnary that links the state and the list of active bits
    """
    Image_Basis = {}
    zero_bit_string = ""
    for bit in range(d1+d2):
        zero_bit_string += "0"
    for i in range(d1):
        bit_string = zero_bit_string[:i] + "1" + zero_bit_string[i+1:]
        for j in range(d2):
            bit_string_state = bit_string[:d1+j] + "1" + bit_string[d1+j+1:]
            Image_Basis[i*d2+j] = [int(bit) for bit in bit_string_state]
    return (Image_Basis)
