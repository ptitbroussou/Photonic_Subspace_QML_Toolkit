...
class BS_Registers_density(nn.Module):
    ...
    def __init__(self, d1, d2, gate_impact, device):
        ...
        self.cos_matrix, self.sin_matrix, self.id_matrix = SinglePhoton_BS_Unitary(d1, d2, gate_impact, device)
    ...
    def forward(self, input_state):
        ...
        Output:
            ...
            input state
        """
        return((self.cos_matrix*torch.cos(self.angle) + self.sin_matrix*torch.sin(self.angle) + self.id_matrix).matmul(input_state).matmul((self.cos_matrix*torch.cos(self.angle) + self.sin_matrix*torch.sin(self.angle) + self.id_matrix).t()))
...
class VQC_Registers_BS_density(nn.Module):
    ...
    def __init__(self, d1, d2, list_gates, device):
        ...
        self.VQC = nn.ModuleList(BS_Registers_density(d1, d2, gates_impact, device) for gates_impact in list_gate_impacts)
...
class BS_Fock_state_vector(nn.Module):
    ...
    def forward(self, input_state):
        ...
        Output:
            ...
            input state
        """
        U = torch.matrix_exp(-1*self.H * self.angle)
...
class BS_Fock_density(nn.Module):
    ...
    def forward(self, input_state):
        ...
        """
        U = torch.matrix_exp(-1*self.H * self.angle)
        return((torch.matrix_exp(-1*self.H * self.angle)).matmul(input_state).matmul((torch.matrix_exp(-1*self.H * self.angle)).t()))
...
class SWAP_Fock_unitary(nn.Module):
    ...
    def __init__(self, n, m, m1, m2, device):
        ...
        self.Unitary_PS = PS_Fock_unitary(n, m, m2, device) # Phase shifter on the second mode
...
class VQC_Fock_BS_state_vector(nn.Module):
    ...
    def __init__(self, n, m, list_gates, device):
        ...
        """
        super().__init__()
        self.VQC = nn.ModuleList(BS_Fock_state_vector(n, m, m1, m2, device) for (m1, m2) in list_gates)
...
class VQC_Fock_BS_density(nn.Module):
    ...
    def __init__(self, n, m, list_gates, device):
        ...
        """
        super().__init__()
        self.VQC = nn.ModuleList(BS_Fock_density(n, m, m1, m2, device) for (m1, m2) in list_gates)
...
class VQC_Fock_BS_density_Parameters(nn.Module):
    ...
    def __init__(self, n, m, list_gates, list_parameters, device):
        ...
        """
        super().__init__()
        self.VQC = nn.ModuleList(BS_Fock_density_Parameter(n, m, m1, m2, list_parameters[i], device) for i,(m1, m2) in enumerate(list_gates))
...
class Equivalent_Unitary_BS_Fock(nn.Module):
    ...
    def __init__(self, n, m, list_gates, list_param, device):
        ...
        """
        super().__init__()
        self.VQC = nn.ModuleList(BS_Fock_unitary(n, m, m1, m2, list_param[i], device) for i, (m1, m2) in enumerate(list_gates))
...
class Equivalent_Unitary_SWAP_Fock(nn.Module):
    ...
    def __init__(self, n, m, list_gates, device):
        ...
        """
        super().__init__()
        self.VQC = nn.ModuleList(SWAP_Fock_unitary(n, m, m1, m2, device) for (m1, m2) in list_gates)
