...
class Basis_Change_Image_to_Fock_density(nn.Module):
    ...
    def forward(self, input_state):
        ...
        Output:
            ...
            the Fock basis (with 2 photons).
        """
        return (self.Passage_matrix.to(torch.float32) @ input_state @ self.Passage_matrix.to(torch.float32).T)
...
class Basis_Change_Image_to_larger_Fock_density(nn.Module):
    ...
    def __init__(self, d1, d2, add1, add2, device):
        ...
        """
        super().__init__()
        self.Passage_matrix = Passage_matrix_Image_larger_Fock(d1, d2, add1, add2, device)
